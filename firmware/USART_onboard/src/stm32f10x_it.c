...
uint8_t Buffer_Rx2[64];
uint8_t Buffer_Tx2[64];
...
void I2C1_EV_IRQHandler(void)
{
    ...
    if (I2C_GetFlagStatus(I2C1, I2C_FLAG_MSL)==RESET)
    {
        ...
        else if(I2C_GetITStatus(I2C1, I2C_IT_TXE)==SET)
        {
            ...
            // Warning: how do we know how much we need to send
        ...
        else if (I2C_GetITStatus(I2C1, I2C_IT_STOPF)==SET)
        {
            switch(Buffer_Rx2[0]){
                case I2C_ID_BROADCAST:
                    ...
                    if((Buffer_Rx2[2]==0x01) && (TYPE!=COLLECTOR))
                    {
                    ...
                    topology[topo_counter++] = (Buffer_Rx2[1] << 8) | Buffer_Rx2[2];
                    ...
                    if(is_splitter(TYPE) && splitter_is_probing 
                            && (next_splitter_ID == 0) && is_splitter(Buffer_Rx2[1]))
                    {
                        next_splitter_ID = Buffer_Rx2[2];
                    }
                    ...
                    if(TYPE == COLLECTOR)
                    {
                        ...
                        if((next_splitter_ID == 0) && is_splitter(Buffer_Rx2[1]))
                        {
                            next_splitter_ID = Buffer_Rx2[2];
                        }

                        // serial output
                        lout.length = snprintf(lout.text,UART_QUEUE_LINE_LENGTH, "i,%02x,%02x\r\n", Buffer_Rx2[1], Buffer_Rx2[2]);
                ...
                case I2C_SPLITTER:
                    ...
                    if((Buffer_Rx2[1] == 0xFF) && (Buffer_Rx2[2] == next_splitter_ID))
                    {
                    ...
                    if(TYPE == COLLECTOR)
                    {
                        topology[topo_counter++] = Buffer_Rx2[1];
